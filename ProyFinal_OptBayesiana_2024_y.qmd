...
| **Hiperparámetro**       | **descripción**                                                                      |**Rango**              | 
|--------------------------|--------------------------------------------------------------------------------------|-----------------------|
| `learning rate`          | controla cuánto ajustar los pesos del modelo con respecto al gradiente de pérdida.   |[1e-5, 1e-1]           |
| `número de unidades (neuronas) en la capa 1`    |   indica el número de neuronas en la capa                     |(16, 32, 64, 128 y 256)|
| `número de unidades (neuronas) en la capa 1`    | indica el número de neuronas en la capa                       |(16, 32, 64, 128 y 256)|  
| `l2_reg`                  | grega una penalización a la función de pérdida para pesos grandes, lo que alienta al modelo a encontrar pesos más pequeños|(1e-6, 1e-2)       | 
| `batch_size`              | el tamaño del lote es la cantidad de imágenes analizadas en una ejecución           |[16, 32, 64, 128]      |
...
La función principal de la biblioteca GPyOpt (*GPyOpt.methods.BayesianOptimization*) que se utiliza para realizar la optimización bayesiana. Crea un optimizador que utiliza un proceso gaussiano (GP) como modelo sustituto para aproximar la función objetivo y mejorarla iterativamente muestreando puntos prometedores.

Ahora se inicia la optimización con un número máximo de ejecuciones. En este caso, 35.
En cada iteración, el optimizador: Selecciona un nuevo valor de entrada de los parametros del NN basado en la función de adquisición (por ejemplo, Mejora esperada). Evalúa la función objetivo en ese punto. Actualiza el modelo sustituto (Proceso Gaussiano) con la nueva información.
...
| `Batch size`             | 64                   |
| `l2_reg`                 | 0.0071               |
| **Mejor MSE Negativo**   | **-0.098**           |


**Optimización Random Search (MNIST)**

En lugar de probar todas las combinaciones (como en Grid Search), tome muestras aleatoriamente del espacio de hiperparámetros. Esto es computacionalmente eficiente y efectivo para espacios más grandes, como se mencionó anteriormente. En el siguiente ejemplo, se utiliza la misma estructura de red neuronal y los parámetros *velocidad de inclinación*, *units_layers01*, *units_layers02*, *l2_reg_rs* y *batch-size* se seleccionan aleatoriamente. Se utiliza el mismo número de ejecuciones. Nuevamente, se utiliza el mismo rango de parámetros para seleccionar parámetros aleatoriamente y configurar el modelo NN. Se utiliza la misma función *build_model()* que en la optimización bayesiana (BO). Se almacenan todos los valores de **mse**. A diferencia de BO, no existe ningún control de mejora integrado, simplemente se selecciona la combinación que da como resultado el **mse** más bajo.
...
| `Batch size`             |  16                  |
| `l2_reg`                 |  0.01                |
| **Mejor MSE Negativo**   | **-0.0252**          |

#### Resultados 

Comparando ambos métodos, el BO selecciona las selecciones más sólidas ya que evalúa los resultados y ajusta los parámetros a la siguiente mejor opción. RS selecciona parámetros aleatoriamente sin criterios ni evaluación. Por lo tanto, no se puede estar seguro de si la mejor solución encontrada es realmente la mejor solución a nivel mundial. Sin embargo, el tiempo para ejecutar RS es considerablemente menor. En el caso documentado, BO tardó aproximadamente entre 66 y 70 minutos en encontrar un mínimo, mientras que RS finalizó esta tarea en entre 6 y 7 minutos. El siguiente gráfico muestra los resultados de ambos métodos. BO encontró hiperparámetros mucho mejores para la estructura del modelo propuesto ($mse_{BO}=-0.098$) que los resultados de RS ($mse_{RS}=-0.0252$). Para concluir la comparación, hay que encontrar un equilibrio entre el tiempo para optimizar y el modelo que mejor se ajusta.

![Modelo con mejores resultados: Bayesian Optimmization vs. Random Search](imgs/BayesOpt_vs_RS_results.png)
