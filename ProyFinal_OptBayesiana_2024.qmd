...
X, Y = data_h.data, data_h.target  # X: características, Y: precios de vivienda
n_features_h = X.shape[1]

# Instantiate an XGBRegressor with default hyperparameter settings
gbr_h = GradientBoostingRegressor(n_estimators = 50, random_state = 0)

# and compute a baseline to beat with hyperparameter optimization 
baseline_h = -cross_val_score(gbr_h, X, Y, scoring='neg_mean_squared_error').mean()
```
...
param_dist_h = {'max_depth': range(1,5),
              ...
              'max_features': range(1, n_features_h),
              'min_samples_split': range(2 ,1000),
              'min_samples_leaf':range(1 ,1000)}

rs_h = RandomizedSearchCV(gbr_h, param_distributions = param_dist_h, 
                        scoring='neg_mean_squared_error', n_iter=25)
...
bds_h = [
    ...
    {'name': 'max_features', 'type': 'discrete', 'domain': list(range(1, n_features_h + 1))},
...
rs_hs = RandomizedSearchCV(
    estimator=gbr_hs,
    param_distributions=param_dist_hs,
...
best_rs_params_hs = rs_hs.best_params_
best_rs_score_hs = -rs_hs.best_score_

print("\nBest parameters from RandomizedSearchCV:")
print(best_rs_params_hs)
print(f"Best neg. MSE from RandomizedSearchCV: {best_rs_score_hs:.5f}")

# Graficar los resultados
# Máximos acumulados de cada iteración
y_rs_hs = -np.maximum.accumulate(rs_hs.cv_results_['mean_test_score'])
y_bo_hs = -np.maximum.accumulate(-np.array(result_hs.func_vals))

# Plotear la comparación
plt.figure(figsize=(6, 4))
plt.plot(y_rs_hs, 'ro-', label='Random search')
